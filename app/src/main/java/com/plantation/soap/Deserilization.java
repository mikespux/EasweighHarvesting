...
public class Deserilization {
    public ArrayList SoapDeserializeArray(Class<?> itemClass, SoapObject object) {
        ...
        for (int i = 0; i < object.getPropertyCount(); i++) {
            try {
                arrayList.add(itemClass.getConstructor(new Class[]{SoapObject.class}).newInstance(object.getProperty(i)));
            ...
            try {
                ...
                if (field.getType().getName() == String.class.getName()) {
                    ...
                    for (int i = 0; i < fieldArray.getPropertyCount(); i++) {
                        Log.d("array", "2.8");
                        Object newObject = genericClass.getConstructor(SoapObject.class).newInstance(fieldArray.getProperty(i));
                        Log.d("array", "3");
                        list.add(newObject);
                        Log.d("array", "4");
                    }
                    ...
                    Constructor constructor = field.getType().getConstructor(SoapObject.class);
                    Object[] objArr = new Object[1];
                    objArr[0] = object.getProperty(field.getName());
